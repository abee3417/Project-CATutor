var editor = CodeMirror.fromTextArea(document.getElementById("codearea"), {
    ...
    smartIndent: function(state, textBefore) {
        ...
        var baseIndent = prevLine.match(/^\s*/)[0];
        // '/'는 정규식(regular expression)을 시작함을 표현, \s는 공백 문자(스페이스, 탭, 줄바꿈 등)에 대응. 따라서 prevLine.match(/^\s*/)[0]은 prevLine 문자열의 시작부터 연속된 공백 문자열 중 가장 긴 것을 반환
        if (/^\s*[\{\(\[]/.test(prevLine)) {
