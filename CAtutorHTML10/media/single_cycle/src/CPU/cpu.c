...
void fetch(CPU* cpu, char* mem){
    ...
    printf(", {\"cycle\": %d, \"pc\": \"0x%x\"", cycle, pc);
	//printf("[Fetch instruction] ");
	printf(", \"ir\": \"0x%08x\"", get_instruction_memory_data(cpu->i_mem));
	return;
}
...
void memory_operation(CPU* cpu, char* mem){
	...
	if (cpu->d_mem.mem_write){
		//printf("[Store] ");
		printf(", \"mem_address\": \"0x%08x\", \"mem_hex_value\": \"0x%08x\", \"mem_dec_value\": \"%d\"", cpu->d_mem.address, cpu->r_file.read_data2, cpu->r_file.read_data2);
		Mem_access_num++;
	}
...
void write_back(CPU* cpu, char* mem){
	...
	if (c_unit_signal.mem_to_reg > 0){
		...
		//printf("[Write Back] ");
		//printf("R[%d] <- 0x%x\n", cpu->r_file.write_reg, get_mux_result(cpu->alu_mem_mux));
	}
	return;
}
...
void pc_update(CPU* cpu){
	...
	int pc_next_j_arr[2] = {get_mux_result(cpu->pc_mux_unit.pc_next_br),
						   	(get_adder_result(cpu->pc_adder)&0xf0000000) | get_shifter_result(cpu->jump_address_shifter)};
