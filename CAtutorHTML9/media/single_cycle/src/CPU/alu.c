...
void set_alu(ALU* alu, int shamt, int input_a, int input_b, char alu_control){
	...
	set_shifter(&(alu->sra_unit), alu->input_b, get_mux_result(alu->shamt_mux), 0, 1);
	set_shifter(&(alu->srl_unit), alu->input_b, get_mux_result(alu->shamt_mux), 0, 0);
	set_shifter(&(alu->sll_unit), alu->input_b, get_mux_result(alu->shamt_mux), 1, 0);
